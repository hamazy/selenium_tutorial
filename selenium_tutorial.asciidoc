...
テスト終了後、`Console` ビューの 停止アイコン image:images/eclipse_terminate.png[""] をクリックし、
Remote Control サーバーを停止しておきます。
...
`selenium-grid-tut/pom.xml` を任意のエディタで編集し、 <<selenium-rc-pom-xml-modification>> と同様に、以下の修正を加えます。
...
Eclipse の `File` &raquo; `Import` メニューから、ウィザードに従って `selenium-grid-tut` ディレクトリーをインポートします。
